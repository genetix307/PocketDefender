...
function gen_wave_auto() {
	...
	gen_count = max(round(random(5+(store.day/25))-random(3.2)),1);
	...
	if 40 +(hud.time/10) + hud.bonus_orc > random(100) {gen_orc(gen_count) exit};
	if 40 +(hud.time/10) + hud.bonus_wizardred > random(100) {gen_wizardred(gen_count) exit};
	if 5 + (store.day/20)  > random(100) and store.day > 25 {gen_green_muskin(gen_count) exit};
	if 25 +(hud.time/10) > random(100) and store.day > 50 {gen_shieldsman(gen_count) exit};
	if 15 +(hud.time/10) > random(100) {gen_shieldsman(1) exit};
	if 50 > random(100) {gen_green_muskin(gen_count) exit};
	}
	}
